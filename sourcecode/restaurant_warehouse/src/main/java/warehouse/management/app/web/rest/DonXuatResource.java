...
public class DonXuatResource {
    ...
    public DonXuatResource(DonXuatService donXuatService, DonXuatRepository donXuatRepository, NhaKhoService nhaKhoService, NhaCungCapService nhaCungCapService, ChiTietDonXuatService chiTietDonXuatService, ChiTietKhoService chiTietKhoService, TaiKhoanService taiKhoanService, NguyenLieuService nguyenLieuService) {
    ...
    public ResponseEntity<?> createDonXuat(@Valid @RequestBody DonXuatRequest donXuatRequest, @RequestHeader("Authorization") String jwt) throws Exception {
        ...
        for (ChiTietPhieuNhapRequest chiTietDonXuat : donXuatRequest.getChiTietPhieuNhapRequestList()) {
            ...
            if (nguyenLieu.isPresent()) {
                ...
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Không tìm thấy nguyên liệu có ID " + chiTietDonXuat.getIdNguyenLieu());
            }
            ...
            chiTietDonXuatDTO.setThanhTien((chiTietDonXuat.getSoLuong() * nguyenLieuDTO.getGiaNhap()) + (chiTietDonXuat.getSoLuong() * nguyenLieuDTO.getGiaNhap() * nguyenLieuDTO.getvAT())/100);
            ...
            if (findChiTietKho != null) {
                ...
                if (findChiTietKho.getSoLuong() > chiTietDonXuat.getSoLuong()) {
                    ...
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Số lượng xuất ra của nguyên liệu " + nguyenLieuDTO.getTenNguyenLieu() + " lớn hơn số lượng có trong kho.");
                }
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Nguyên liệu " + nguyenLieuDTO.getTenNguyenLieu() +" không có sẵn trong kho.");
            }
    ...
    public ResponseEntity<?> createDonXuatByExcelFile(@ModelAttribute DonXuatExcelRequest donXuatExcelRequest, @RequestHeader("Authorization") String jwt) throws Exception {
        DonXuatDTO donXuatDTO = new DonXuatDTO();
        DonXuatDTO donXuatDTO1 = new DonXuatDTO();
        donXuatDTO.setNgayLap(Instant.now());
        donXuatDTO.setNguoiXacNhan(null);
        ...
        for (Row chiTietDonXuat : sheet) {
            ...
            chiTietDonXuatDTO.setNguyenLieu(nguyenLieuDTO);
            chiTietDonXuatDTO.setThanhTien((soLuong * nguyenLieuDTO.getGiaNhap()) + (soLuong * nguyenLieuDTO.getGiaNhap() * nguyenLieuDTO.getvAT())/100);
            ...
            Optional<ChiTietKhoDTO> chiTietKhoDTO;
            if (findChiTietKho != null) {
                if (findChiTietKho.getSoLuong() > soLuong) {
                } else if (Objects.equals(findChiTietKho.getSoLuong(), soLuong)) {
                } else {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Số lượng xuất ra của nguyên liệu " + nguyenLieuDTO.getTenNguyenLieu() + " lớn hơn số lượng có trong kho.");
                }
            } else {
...
//    }
    ...
    public ResponseEntity<?> getUUID(@PathVariable Long id, @RequestHeader("Authorization") String jwt) throws Exception {
        ...
        for (ChiTietDonXuat chiTietDonXuat : chiTietDonXuatList) {
            ChiTietKho findChiTietKho = chiTietKhoService.findByNguyenLieuIdAndNhaKhoId(chiTietDonXuat.getNguyenLieu().getId(), result.getNhaKho().getId());
            Optional<ChiTietKhoDTO> chiTietKhoDTO;
            if (findChiTietKho != null) {
                chiTietKhoDTO = chiTietKhoService.findOne(findChiTietKho.getId());
                ...
                if (findChiTietKho.getSoLuong() > chiTietDonXuat.getSoLuong()) {
                    ...
                    throw new Exception("Số lượng xuất ra của nguyên liệu " + chiTietDonXuat.getNguyenLieu().getTenNguyenLieu() + " lớn hơn số lượng có trong kho.");
                }
            } else {
